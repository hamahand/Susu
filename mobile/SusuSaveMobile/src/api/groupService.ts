...
export const groupService = {
   ...
   */
  async inviteMember(groupId: number, phoneNumber: string): Promise<any> {
   ...
   */
  async getPendingInvitations(groupId: number): Promise<any[]> {
    const response = await apiClient.get(`/groups/${groupId}/invitations`);
    return response.data;
  },

  /**
   * Set or remove admin role for a member (creator only)
   */
  async setMemberAdmin(groupId: number, userId: number, isAdmin: boolean): Promise<any> {
    const response = await apiClient.post(`/groups/${groupId}/members/${userId}/set-admin`, {
      is_admin: isAdmin,
    });
    return response.data;
  },
   ...
   */
  async updateGroupPrivacy(groupId: number, settings: GroupPrivacySettings): Promise<Group> {
    const response = await apiClient.patch<Group>(`/groups/${groupId}/privacy`, settings);
    return response.data;
  },
};
